...
export default function Person({personId}) {
  ...
  const {data: person, error, isIdle, isLoading, isError, isSuccess} = useQuery('person', async () => getPersonById(personId), {enabled: !router.isFallback});
  if (isIdle || isLoading) {
    return (
      <>
    ...
    return (
      <>
        <Head><title>{person.name} | Pop Central </title></Head>
        <div css={{
          ...
          <div css={{
            ...
            <div css={{
              ...
              {person.credits.map((credit, index) => (
                <Link key={index} href={`/${credit.project.mediaType === 'movie' ? 'movies' : 'tv'}/${credit.project.id}`} passHref>
                  <a css={{
                    textDecoration: 'none',
